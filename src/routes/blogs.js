...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const posts = await getPostsAuth(req.user.id);
      ...
      res.status(200).json({
        ...
        data: await getPostsAuthPagination(req.user.id, page || 1, limit || 10),
      });
    } catch (error) {
      console.log(error.message);
      return next(error);
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const posts = await getBannedPosts(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getBannedPostsPagination(
          req.user.id,
          page || 1,
          limit || 10
        ),
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const blogs = await getBlogsAuth(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getBlogsAuthPagination(req.user.id, page || 1, limit || 10),
      });
    } catch (error) {
      console.log(error.message);
      return next(error);
...
// Get account blogs
router.get("/blogs/account/:id", async (req, res, next) => {
  const { id } = req.params;
  const { page, limit } = req.query;
  ...
  try {
    ...
    if (accountFound.isBanned === true) {
      return res.status(400).json({
        statusCode: 400,
        msg: `The account is banned! You can not access to its blogs...`,
      });
    }

    const blogs = await getBlogAccountBlogs(id);
    ...
    res.status(200).json({
      ...
      totalPages,
      page: parseInt(page) || 1,
      data: await getBlogAccountBlogsPagination(id, page || 1, limit || 10),
    });
  } catch (error) {
    console.log(error.message);
    return next(error);
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const blogs = await getNotBannedBlogs(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getNotBannedBlogsPagination(
          req.user.id,
          page || 1,
          limit || 10
        ),
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const blogs = await getBannedBlogs(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getBannedBlogsPagination(
          req.user.id,
          page || 1,
          limit || 10
        ),
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const accounts = await getNotBannedAccounts(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getNotBannedAccountsPagination(
          req.user.id,
          page || 1,
          limit || 10
        ),
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      const accounts = await getBannedAccounts(req.user.id);
      ...
      res.status(200).json({
        ...
        totalPages,
        page: parseInt(page) || 1,
        data: await getBannedAccountsPagination(
          req.user.id,
          page || 1,
          limit || 10
        ),
